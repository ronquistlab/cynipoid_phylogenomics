...
draw_tree3 <- function(tree, hexp, cyn_mono=TRUE, leg=TRUE, treescale_height=8.0)
{
    require(ggtree)
    require(ape)
    ...
    for(i in 1:length(scf[,2])){
        if(scf[i,2]<=BREAKS[1]){
            COLOR=c(COLOR,Grey_Palette[1])
        }
        if(scf[i,2]>BREAKS[1] & scf[i,2]<=BREAKS[2]){
            COLOR=c(COLOR,Grey_Palette[2])
        }
        if(scf[i,2]>BREAKS[2] & scf[i,2]<=BREAKS[3]){
            COLOR=c(COLOR,Grey_Palette[3])
        }
        if(scf[i,2]>BREAKS[3] & scf[i,2]<=BREAKS[4]){
            COLOR=c(COLOR,Grey_Palette[4])
        }
        if(scf[i,2]>BREAKS[4] & scf[i,2]<=BREAKS[5]){
            COLOR=c(COLOR,Grey_Palette[5])
        }
        if(scf[i,2]>BREAKS[5]){
            COLOR=c(COLOR,Grey_Palette[6])
...
cowplot::plot_grid(p1,p2,p3,ncol=3,labels=labels, label_size=10, hjust=0.0) + theme(plot.margin=unit(c(3,3,3,3), "pt"))

ggsave("Fig_S5.svg", device="svg")
