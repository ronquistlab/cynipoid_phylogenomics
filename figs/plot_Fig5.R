...
draw_tree <- function(tree, hexp, cyn_mono=TRUE, leg=TRUE, treescale_height=8.0)
{
    ...
    # Read the tip labels and desired display labels
    source("names.R")
    ...
    ggtree(tree, aes(color = group), ladderize = TRUE) +
        geom_tree(size=0.6, key_glyph=draw_key_rect) +
        ...
        scale_color_manual(values = c("green3", "black", "black"), na.value = "black",
                           name = "Larval feeding mode",
